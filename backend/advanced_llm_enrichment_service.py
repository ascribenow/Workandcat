...
class AdvancedLLMEnrichmentService:
    ...
    def _verify_response_quality(self, response_data: Dict[str, Any], model_used: str) -> bool:
        """
        Verify that response meets reasonable quality standards
        ...
        # Check for basic completeness (not overly strict)
        if 'right_answer' in response_data:
            answer = response_data['right_answer']
            if not answer or len(answer.strip()) < 5:  # Very basic requirement
                quality_issues.append("Right answer missing or too brief")
        
        # Only reject truly generic/empty content
        if 'category' in response_data:
            category = response_data['category'] or ""
            if category.lower() in ['', 'general', 'basic', 'unknown']:
                quality_issues.append(f"Empty or meaningless category: {category}")
                
        # Only check for completely empty concepts
        if 'core_concepts' in response_data:
            try:
                ...
                if not concepts or (isinstance(concepts, list) and len(concepts) == 0):
                    quality_issues.append("Core concepts list is empty")
            except:
                pass
                
        # Only reject completely empty solution methods
        if 'solution_method' in response_data:
            method = response_data['solution_method'] or ""
            if not method.strip():
                quality_issues.append("Solution method is empty")
        
        if quality_issues:
            logger.warning(f"⚠️ Quality issues from {model_used}: {quality_issues}")
            return False
        
        logger.info(f"✅ Quality standards met by {model_used} - content accepted")
        return True
