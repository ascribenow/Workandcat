...
# =============================================================================
# STAGE 5B: QUALITY VERIFICATION (Relaxed - Skip CSV Field Checks)
# =============================================================================

@stages_router.post("/stage5b-quality-relaxed", response_model=StageResponse)
async def stage5b_quality_relaxed(request: StageRequest):
    """
    STAGE 5B: Quality Verification (Relaxed)
    Sets: quality_verified = True if answer_match = True and LLM fields complete
    SKIPS: CSV field completeness checks
    """
    db = SessionLocal()
    try:
        logger.info("🎯 STAGE 5B: Quality Verification (Relaxed)")
        
        # Get questions with answer_match but need quality verification
        if request.question_ids:
            questions = db.execute(
                select(Question).where(Question.id.in_(request.question_ids))
            ).scalars().all()
            ...
            questions = db.execute(
                select(Question)
                .where(and_(
                    Question.answer_match.isnot(None),
        ...
        for question in questions:
            try:
                logger.info(f"🔄 Processing {question.id[:8]}... - Quality Verification (Relaxed)")
                
                # Relaxed quality verification - only check essential fields
                quality_verified = True
                failure_reasons = []
                
                # Check 1: answer_match = True
                if not question.answer_match:
                    quality_verified = False
                    failure_reasons.append("answer_match=False")
                
                # Check 2: concept_extraction_status = 'completed'
                if question.concept_extraction_status != 'completed':
                    quality_verified = False
                    failure_reasons.append(f"concept_extraction_status='{question.concept_extraction_status}'")
                
                # Check 3: Essential LLM fields present
                essential_llm_fields = [
                    'right_answer', 'category', 'subcategory', 'type_of_question',
                    'core_concepts', 'difficulty_score'
                ]
                
                for field in essential_llm_fields:
                    value = getattr(question, field, None)
                    if not value or value in ['', 'null', 'None', None]:
                        quality_verified = False
                        failure_reasons.append(f"missing_{field}")
                
                # Update quality verification
                question.quality_verified = quality_verified
                if quality_verified:
                    question.is_active = True
                ...
                if quality_verified:
                    ...
                    failed_count += 1
                    logger.info(f"❌ Quality failed: {failure_reasons}")
        ...
        return StageResponse(
            success=True,
            stage="stage5b_quality_relaxed",
            processed=processed,
            success_count=success_count,
            failed_count=failed_count,
            details={
                "fields_set": ["quality_verified", "is_active"],
                "verification_rate": f"{(success_count/processed)*100:.1f}%" if processed > 0 else "0%",
                "criteria": "answer_match=True + concept_extraction_status='completed' + essential_LLM_fields"
            }
        )
...
# =============================================================================
# STAGE 5C: QUALITY VERIFICATION (Strict - All 22 Criteria)
# =============================================================================

@stages_router.post("/stage5c-quality-strict", response_model=StageResponse)
async def stage5c_quality_strict(request: StageRequest):
    """
    STAGE 5C: Quality Verification (Strict)
    Sets: quality_verified = True only if ALL 22 criteria met (including CSV fields)
    """
    db = SessionLocal()
    try:
        logger.info("🎯 STAGE 5C: Quality Verification (Strict)")
        
        # Get questions for strict quality verification
        if request.question_ids:
            questions = db.execute(
                select(Question).where(Question.id.in_(request.question_ids))
            ).scalars().all()
            ...
            questions = db.execute(
                select(Question)
                .where(Question.answer_match.isnot(None))
                .limit(request.limit)
            ).scalars().all()
        ...
        for question in questions:
            try:
                logger.info(f"🔄 Processing {question.id[:8]}... - Quality Verification (Strict)")
                
                # Use the full quality verification process
                quality_result = await regular_questions_enrichment_service._perform_quality_verification(
                    question.stem,
                    {
                        'right_answer': question.right_answer,
                ...
                # Update fields
                question.quality_verified = quality_result.get('quality_verified', False)
                question.answer_match = quality_result.get('answer_match', question.answer_match)
                ...
                if question.quality_verified:
                    success_count += 1
                    logger.info(f"✅ Strict quality verified: TRUE")
                else:
                    failed_count += 1
                    logger.info(f"❌ Strict quality failed")
                
            except Exception as e:
                failed_count += 1
                logger.error(f"❌ Error in strict quality verification for {question.id[:8]}: {e}")
        
        return StageResponse(
            success=True,
            stage="stage5c_quality_strict",
            processed=processed,
            success_count=success_count,
            failed_count=failed_count,
            details={
                "fields_set": ["quality_verified", "is_active", "answer_match"],
                "verification_rate": f"{(success_count/processed)*100:.1f}%" if processed > 0 else "0%",
                "criteria": "All 22 criteria including CSV field completeness"
            }
        )
