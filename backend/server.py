...
# Rate limiting functions removed - not needed for current implementation
...
async def deprecated_session_start():
    ...
    telemetry_service.emit_metric("legacy.endpoint.sessions_start_410", 1, {
        "deprecated_at": now_ist().isoformat(),
...
async def log_question_action(
    ...
    try:
        ...
        try:
            ...
            if not question:
                logger.warning(f"⚠️ Question {log_data.question_id} not found in Blueprint session data for session {log_data.session_id[:8]}")
                return {
                    "success": False,
                    "message": "Question not found in current Blueprint session",
...
async def get_dashboard_adaptive_insights(user_id: str = Depends(get_current_user)):
    ...
    try:
        ...
        logger.error(f"Error getting concept insights for user {user_id[:8]}: {e}")
