...
async def enrich_question_background(question_id: str, hint_category: str = None, hint_subcategory: str = None):
    ...
    try:
        ...
        try:
            ...
            # Commit LLM enrichment with explicit session flush
            ...
            if verification_query and verification_query.answer != "To be generated by LLM":
                logger.info(f"✅ Step 1 completed: LLM enrichment for question {question_id} - VERIFIED")
            else:
                logger.warning(f"⚠️ Step 1 commit verification failed for question {question_id}")
            ...
            try:
                ...
                logger.error(f"Failed to commit fallback state: {commit_error}")
        ...
        try:
            ...
            if question and question.subcategory:
                ...
                # Verify PYQ analysis commit
                verification_query = db.query(Question).filter(Question.id == question_id).first()
                if verification_query and verification_query.pyq_frequency_score == pyq_score:
                    logger.info(f"✅ Step 2 completed: PYQ frequency analysis for question {question_id} (score: {pyq_score:.3f}) - VERIFIED")
                else:
                    logger.warning(f"⚠️ Step 2 commit verification failed for question {question_id}")
            ...
            try:
                ...
                db.flush()
                db.commit()
                logger.info(f"🔧 Applied emergency PYQ fallback for question {question_id}")
            except Exception as fallback_error:
                logger.error(f"PYQ fallback also failed: {fallback_error}")
