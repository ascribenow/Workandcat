...
@api_router.post("/admin/fix-pyq-categories", dependencies=[Depends(require_admin)])
async def fix_pyq_categories(
    db: AsyncSession = Depends(get_async_compatible_db)
):
    ...
    try:
        ...
        for subcategory, category in SUBCATEGORY_TO_CATEGORY.items():
            # Update questions with this subcategory and null category
            result = await db.execute(
                update(PYQQuestion)
                .where(
            ...
            questions_updated = result.rowcount
            if questions_updated > 0:
                results.append(f"{subcategory} → {category}: {questions_updated} questions")
        ...
        verification_result = await db.execute(
            select(PYQQuestion)
            .where(PYQQuestion.category.isnot(None))
            .limit(5)
        )
        verification_questions = verification_result.scalars().all()
        ...
        for q in verification_questions:
            verification_data.append({
                "category": q.category,
                "subcategory": q.subcategory,
                "stem_preview": q.stem[:50] + "..."
            })
        
        return {
            "success": True,
            "message": f"Successfully updated {updated_count} PYQ questions with category data",
            "updated_count": updated_count,
            "mapping_results": results,
            "verification_sample": verification_data
        }
        ...
        for i, row in enumerate(processed_rows):
            try:
                ...
                question = Question(
                    ...
                    stem=stem,
                    answer=admin_answer or "Not provided", # ADMIN FIELD - PROTECTED FROM CSV
                    solution_approach=solution_approach, # ADMIN FIELD - FROM CSV
                    detailed_solution=detailed_solution, # ADMIN FIELD - FROM CSV
                    principle_to_remember=principle_to_remember, # ADMIN FIELD - FROM CSV
                    snap_read=snap_read, # NEW ADMIN FIELD - FROM CSV
                    mcq_options=mcq_options, # ADMIN FIELD - FROM CSV
                    # LLM will populate these fields during immediate enrichment (SAME AS PYQ)
                    right_answer=None,  # To be generated by LLM enrichment
                    category=None, # To be generated by LLM enrichment
                    subcategory="To be classified by LLM",  # To be generated by LLM enrichment
                    type_of_question="To be classified by LLM", # To be generated by LLM enrichment
                    difficulty_band=None, # To be generated by LLM enrichment
                    difficulty_score=None, # To be generated by LLM enrichment
                    quality_verified=False, # To be set by LLM enrichment
                    core_concepts=None, # To be generated by LLM enrichment
                    solution_method=None, # To be generated by LLM enrichment
                    concept_difficulty=None, # To be generated by LLM enrichment
                    operations_required=None, # To be generated by LLM enrichment
                    problem_structure=None, # To be generated by LLM enrichment
                    concept_keywords=None, # To be generated by LLM enrichment
                    # Image support fields
                    has_image=has_image,
                    image_url=image_url,
                    # image_alt_text REMOVED as per requirements
                    # Metadata 
                    source="Admin CSV Upload - Refactored Workflow",
