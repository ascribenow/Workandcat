...
async def upload_questions_csv(
    ...
    """Upload questions from CSV file with Google Drive image support"""
    try:
        ...
        # Process Google Drive image URLs if present
        logger.info(f"Processing {len(csv_rows)} rows from CSV with potential Google Drive images")
        ...
        for i, row in enumerate(processed_rows):
            ...
            question = Question(
                topic_id=topic.id,
                stem=stem,
                answer=answer or "To be generated by LLM",  # Allow LLM to generate if missing
                solution_approach="",
                detailed_solution="",
                subcategory=subcategory or category or "General",
                type_of_question="",  # Empty for CSV uploads, can be enriched later
                tags=[],
                source=source or "CSV Upload with Google Drive Images",
                # Image support fields
                has_image=has_image,
                image_url=image_url,
        ...
        logger.info(f"CSV upload completed: {questions_created} questions created, {images_processed} with images")
