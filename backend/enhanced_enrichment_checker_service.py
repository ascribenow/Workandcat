...
class EnhancedEnrichmentCheckerService:
    ...
    async def _assess_pyq_question_enhanced(self, question: PYQQuestion) -> Dict[str, Any]:
        ...
        try:
            ...
            # 2. CHECK FOR PLACEHOLDER TEXT IN BASIC FIELDS
            
            # Check if answer is still placeholder
            if not question.answer or question.answer == "To be generated by LLM":
                failed_criteria.append("answer_still_placeholder")
                quality_issues.append("Answer field contains placeholder text - must be generated")
                is_acceptable = False
            
            # Check if subcategory is still placeholder
            if not question.subcategory or question.subcategory == "To be classified by LLM":
                failed_criteria.append("subcategory_still_placeholder")
                quality_issues.append("Subcategory field contains placeholder text - must be classified")
                is_acceptable = False
            
            # Check if type_of_question is still placeholder
            if not question.type_of_question or question.type_of_question == "To be classified by LLM":
                failed_criteria.append("type_of_question_still_placeholder")
                quality_issues.append("Type of question field contains placeholder text - must be classified")
                is_acceptable = False
            
            # 3. CHECK CANONICAL TAXONOMY COMPLIANCE (only if not placeholder)
            ...
            # Type of question validation for PYQ (only if not placeholder)
            if (category_for_subcategory and category_for_subcategory in self.canonical_question_types and 
                question.type_of_question and question.type_of_question != "To be classified by LLM"):
                valid_types = self.canonical_question_types[category_for_subcategory]
                if question.type_of_question not in valid_types:
                    failed_criteria.append("canonical_taxonomy_type_violation")
