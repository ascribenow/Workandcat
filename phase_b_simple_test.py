...
def test_idempotency_key_requirement(token, user_id):
    ...
    try:
        ...
        response = requests.post(
            "https://learn-planner-1.preview.emergentagent.com/api/adapt/plan-next",
            json=test_data,
            headers=headers,
            timeout=10,
            verify=False
...
def test_endpoint_accessibility(token, user_id):
    ...
    try:
        response = requests.get(
            f"https://learn-planner-1.preview.emergentagent.com/api/adapt/pack?user_id={user_id}&session_id=nonexistent",
            headers=headers,
            timeout=10,
            verify=False
        )
    ...
    try:
        response = requests.post(
            "https://learn-planner-1.preview.emergentagent.com/api/adapt/mark-served",
